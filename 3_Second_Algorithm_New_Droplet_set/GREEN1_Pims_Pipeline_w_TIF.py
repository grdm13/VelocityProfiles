...
def components_n_Canny(img):
    ...
    my_df.to_csv(f' pims matrix from {image_counter}.csv', header=False, index=False)  # save as csv


images = pims.open('/Users/georgedamoulakis/PycharmProjects/VelocityProfiles/tamalPNG/*.png')  # many PNGs with sequential names


for i in range(len(images)):
    image_counter = image_counter + 1
    components_n_Canny(images[i])
